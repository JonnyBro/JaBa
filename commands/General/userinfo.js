...
class Userinfo extends Command {
	...
	async run(message, args, data) {
		...
		if (member) {
			...
			embed.addField(this.client.customEmojis.roles + " " + message.translate("common:ROLES"), (member.roles.size > 10 ? member.roles.cache.filter((r) => r.name !== "@everyone").map((r) => r).slice(0, 9).join(", ") + " " + message.translate("general/userinfo:MORE_ROLES", {
